...
for i in range(len(b)):
    print(". ", end='', flush=True)
    if b[i] == 0:
        _alphai=E.isogeny(kernel=s[i], algorithm="factored")
        Curves.append(_alphai.codomain())
    elif b[i] == 1:
        _betai=Es.isogeny(kernel=s[i], algorithm="factored")
        Curves.append(_betai.codomain())
print()
...
for _Ei in Curves:
    print(". ", end='', flush=True)
    re, im=_Ei.j_invariant()
    _r += long_to_bytes(re.lift()) + long_to_bytes(im.lift())
print()
